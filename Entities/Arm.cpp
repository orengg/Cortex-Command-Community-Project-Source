    ...
    if (!m_pParent) {
        ...
        else
        {
            ...
            else
            {
                ...
                if (handTarget.GetMagnitude() <= m_MaxLength || !m_WillIdle)
                {
		...
		float animProgress = (m_HandOffset.GetMagnitude() / m_MaxLength) * static_cast<float>(m_FrameCount);
		int newFrame = static_cast<int>(floorf(animProgress));
#if defined DEBUG_BUILD || defined MIN_DEBUG_BUILD
		RTEAssert(newFrame <= m_FrameCount, "Arm sprite frame number is too high."); // If newFrame > m_FrameCount then there's a bug, it's better to crash.
#endif
